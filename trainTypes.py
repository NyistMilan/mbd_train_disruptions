# Compare train types performances
...
avg_delay_by_traintype_year = (
    per_rdt
    .groupBy("year", "Service:Type")
    .agg(
...
status_counts = (
    per_rdt
    .groupBy("year", "Service:Type", "service_status")
    .count()
)
...
ratios = (
    status_counts
    .join(total_counts, ["year", "Service:Type"])
    .withColumn(
        "ratio",
...
final_by_year = (
    avg_delay_by_traintype_year
    .join(ratios_pivot, ["year", "Service:Type"], "left")
)
...
final_output \
    ...
    .mode("overwrite") \
    .option("header", "true") \
    .csv("/user/s3063496/final_project/results/traintype_by_year")
